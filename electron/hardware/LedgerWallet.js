...
const LEDGER_PATHS = {
	[Blockchains.PICOIO]:(index = 0) => `44'/194'/0'/0/${index}`,
	[Blockchains.ETH]:(index = 0) => `44'/60'/0'/0/${index}`,
}

let encoderOptions, picojsUtil;
...
class LedgerWallet {
	...
	static async setup(){
		...
		picojsUtil = new Api(encoderOptions);
	...
	static availableBlockchains(){
		return [Blockchains.PICOIO, Blockchains.ETH];
	}
...
class LedgerAPI {
	...
	constructor(blockchain){
		...
		switch(this.blockchain){
			case Blockchains.PICOIO: scrambleKey = "e0s"; break;
	...
	[`getAddress`+Blockchains.PICOIO](index, boolChaincode = false){
	...
	[`signTransaction`+Blockchains.PICOIO](publicKey, transaction, abi, network){
		...
		try {
			rawTx = serializePicojs(network, transaction);
		} catch(e){
			console.error('e', e);
			return null;
		}
	...
	[`getAppConfiguration`+Blockchains.PICOIO](){
...
const serializePicojs = (network, transaction) => {

	const types = {};
	picojsUtil.abiTypes.forEach((value, key) => types[key] = value);
	picojsUtil.transactionTypes.forEach((value, key) => types[key] = value);
